						...
						char *output_file, char *format)
{
	...
	for (j = 0; j < outRasterY; j++)
	{
		...
		{
			*lacunarityPtr = lacunarity_in_window(data, rasterX, rasterY, f3d,
												  gbox, 
												  i, j, mwin, mwin);
			lacunarityPtr++;
							 ...
							 int mwinX, int mwinY, int mwinW, int mwinH)
{
	...
	for (j = 0; j < nGlidingStepsY; j++)
	{
		...
		for (i = 0; i < nGlidingStepsX; i++)
		{
			...
			//intensitySumPtr = ...;
			//intensitySumPtr = intensitySum + (i * j * nLevels);
			// Il y a une erreur là dedans. C'est quelque chose avec (j*nGlidingSteps) + i etc.
